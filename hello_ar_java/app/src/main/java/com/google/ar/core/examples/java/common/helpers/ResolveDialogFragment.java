...
public class ResolveDialogFragment extends DialogFragment {
  ...
  private LinearLayout getDialogLayout() {
    ...
    locationField = new EditText(context);
    // Only allow numeric input.
    locationField.setInputType(InputType.TYPE_CLASS_NUMBER);
    locationField.setLayoutParams(
        new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
    // Set a max length for the input text to avoid overflows when parsing.
    locationField.setFilters(new InputFilter[] {new InputFilter.LengthFilter(MAX_FIELD_LENGTH)});
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    builder
        ...
        .setPositiveButton(
            "OK",
            (dialog, which) -> {
              Editable shortCodeText = locationField.getText();
